...
def vllm_generate(raft_sample_K,model,tokenizer,prompts,temperature,max_tokens):
    device = model.policy.device
    model.to('cpu')
    torch.cuda.empty_cache()
    policy = model.policy
    sampling_params = SamplingParams(temperature=temperature, top_p=0.95, n=raft_sample_K, max_tokens=max_tokens, logprobs =1,detokenize=False,ignore_eos=False, seed=random.randint(1, 5000)) # such seed can avoid overfitting
    ...
    def train(self):
        ...
        for update in range(1, args.num_total_batches + 1):
            ...
            with torch.no_grad():
                ...
                # each quesiton_string with args.raft_sample_K samples
                responses = vllm_generate(args.raft_sample_K, model, self.tokenizer, quesiton_string, args.temperature, max_tokens=generation_config.max_new_tokens)

                responses_decoded = self.tokenizer.batch_decode(responses)

                quesiton_string_N = [item_s for item_s in quesiton_string for _ in range(args.raft_sample_K)]
                ...
                with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
                    ...
                    queries = torch.repeat_interleave(queries, repeats = args.raft_sample_K, dim=0)
                ...
                # 5. compute RAFT rlhf_reward =  kl + scores
                rlhf_reward = rewards.sum(1)

                # save for logger
                log_rlhf_reward = rlhf_reward.clone()

                # 6. keep those max reward RAFT samples, abandon the rest ------------------------
                rlhf_reward = rlhf_reward.view(-1, args.raft_sample_K)
                _, max_indices = torch.max(rlhf_reward, dim=1)

                max_indices = torch.randint(0, args.raft_sample_K, (batch_size,))

                responses = responses.view(batch_size,args.raft_sample_K,-1)
                responses = responses[torch.arange(batch_size), max_indices]
                query_responses = query_responses.view(batch_size,args.raft_sample_K,-1)
                query_responses = query_responses[torch.arange(batch_size), max_indices]
                padding_mask = padding_mask.view(batch_size,args.raft_sample_K,-1)
                padding_mask = padding_mask[torch.arange(batch_size), max_indices]
                ...
                for mini_batch_start in tqdm(range(0, args.local_batch_size, args.local_mini_batch_size),desc='loss For&backward of 1 epoch of samples'):
                    ...
                    for micro_batch_start in range(0, args.local_mini_batch_size, args.per_device_train_batch_size):
                        ...
                        with accelerator.accumulate(model):
                            ...
                            pg_loss = -new_logprobs.sum(1).mean()
                            loss = pg_loss
                    ...
                    del (
                        new_logprobs, 
                        pg_loss, loss, entropy,
                        mb_responses, mb_query_responses,
                    )
                    torch.cuda.empty_cache()
