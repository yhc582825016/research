...
def vllm_generate(model,tokenizer,prompts,temperature,max_tokens):
    ...
    policy = model.policy
    sampling_params = SamplingParams(temperature=temperature, top_p=0.95, n=1, max_tokens=max_tokens, logprobs =1,detokenize=False,ignore_eos=False, seed=random.randint(1, 5000))# such seed can avoid overfitting
    ...
    for o in outputs_ALL:
        ...
        baseline_response+=[tokens]

    baseline_response = torch.stack(baseline_response, dim=0)

    del llm
    torch.cuda.empty_cache()
    model.to(device)

    return response,baseline_response
    ...
    def train(self):
        ...
        for update in range(1, args.num_total_batches + 1):
            ...
            with torch.no_grad():
                ...
                batch_size_qs = queries.shape[0]
   

                quesiton_string = self.tokenizer.batch_decode(queries)
                quesiton_string = [q.replace('[PAD]','') for q in quesiton_string]
                
                responses, baseline_responses = vllm_generate(model,self.tokenizer, quesiton_string, args.temperature, max_tokens=generation_config.max_new_tokens)
                res_and_bs = torch.cat((responses,baseline_responses),dim=0)
            ...
            with torch.no_grad():
                ...
                rlhf_reward = mean_non_score_reward + scores_reward.mean()
                ...
                metrics["eval_objective/scores_old"] = self.accelerator.gather(scores_reward).mean().item()
